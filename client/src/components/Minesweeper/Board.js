...
export default class Board extends Component {
  ...
  handleWin() {
    ...
    let initials =
      document.getElementById("letter-1").value +
      document.getElementById("letter-2").value +
      document.getElementById("letter-3").value;
    let uppInitials = initials.toUpperCase();
    console.log(initials);
    // console.log(uid);
    API.postMineSweeper(score, uid, uppInitials)
      .then(response => {
        console.log(response);
      })
  ...
  renderBoard = (rows, cols) => {
    ...
    if (this.state.win && this.props.uid) {
      ...
      actionButton = (
        <Button variant="primary" onClick={this.handleWin}>
          Save
        </Button>
      );
    ...
    return (
      <div className="board">
        ...
        <div className="container timer">
          <div>
            Time: {ms(this.state.time, { keepDecimalsOnWholeSeconds: true })}
          </div>
