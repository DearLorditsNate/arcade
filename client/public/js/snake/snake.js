...
class Snake {
  ...
  randomTarget() {
    ...
    while (assigned === false) {
      ...
      if (board[randomRow][randomColumn] !== "occupied") {
        ...
        drawSquare(
          currentIndexedTargetSpot.x * SQ,
          currentIndexedTargetSpot.y * SQ,
          targetColor
        );
        assigned = true;
      }
  ...
  checkIfTargetReached() {
    if (
      this.x === currentIndexedTargetSpot.x &&
      this.y === currentIndexedTargetSpot.y
    ) {
      score++;
  ...
  checkForBoundaryInfraction() {
    if (this.x < 0 || this.y < 0 || this.x === column || this.y === row) {
      ...
      var firebaseuid = document
        .getElementById("snakewrapper")
        .getAttribute("data-id");
      console.log(firebaseuid);
      if (firebaseuid) {
        $("#snake-save-modal").modal("show");
        $("#snake-save-modal-title").text(`Your score: ${score}`);
